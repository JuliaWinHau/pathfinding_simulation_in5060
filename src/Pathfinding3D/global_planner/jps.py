...
class JPS(AStar):
    ...
    def jump(self, node: Node, motion: Node):
        ...
        # XZ diagonal
        if motion.x and motion.z and not motion.y:
            x_dir = Node((motion.x, 0, 0), None, 1, None)
        ...
        # YZ diagonal
        if motion.y and motion.z and not motion.x:
            y_dir = Node((0, motion.y, 0), None, 1, None)
            z_dir = Node((0, 0, motion.z), None, 1, None)
        ...
        if motion.x and motion.y and motion.z:
            xy_dir = Node((motion.x, motion.y, 0), None, sqrt(2), None)
            xz_dir = Node((motion.x, 0, motion.z), None, sqrt(2), None)
            yz_dir = Node((0, motion.y, motion.z), None, sqrt(2), None)
    ...
    def detectForceNeighbor(self, node, motion):
        ...
        if x_dir and not y_dir and not z_dir:
            if (x, y + 1, z) in obs and (x + x_dir, y + 1, z) not in obs: return True
            if (x, y - 1, z) in obs and (x + x_dir, y - 1, z) not in obs: return True
            if (x, y, z + 1) in obs and (x + x_dir, y, z + 1) not in obs: return True
            if (x, y, z - 1) in obs and (x + x_dir, y, z - 1) not in obs: return True

        # Y-only
        if y_dir and not x_dir and not z_dir:
            if (x + 1, y, z) in obs and (x + 1, y + y_dir, z) not in obs: return True
            if (x - 1, y, z) in obs and (x - 1, y + y_dir, z) not in obs: return True
            if (x, y, z + 1) in obs and (x, y + y_dir, z + 1) not in obs: return True
            if (x, y, z - 1) in obs and (x, y + y_dir, z - 1) not in obs: return True

        # Z-only
        if z_dir and not x_dir and not y_dir:
            if (x + 1, y, z) in obs and (x + 1, y, z + z_dir) not in obs: return True
            if (x - 1, y, z) in obs and (x - 1, y, z + z_dir) not in obs: return True
            if (x, y + 1, z) in obs and (x, y + 1, z + z_dir) not in obs: return True
            if (x, y - 1, z) in obs and (x, y - 1, z + z_dir) not in obs: return True

        # 2-axis diagonals
        # XY-diagonal
        if x_dir and y_dir and not z_dir:
            # same-plane checks (your 2D rules in XY)
            if (x - x_dir, y, z) in obs and (x - x_dir, y + y_dir, z) not in obs: return True
            if (x, y - y_dir, z) in obs and (x + x_dir, y - y_dir, z) not in obs: return True
            # also consider Z sides
            if (x, y, z + 1) in obs and (x + x_dir, y + y_dir, z + 1) not in obs: return True
            if (x, y, z - 1) in obs and (x + x_dir, y + y_dir, z - 1) not in obs: return True

        # XZ-diagonal
        if x_dir and z_dir and not y_dir:
            # same-plane checks in XZ
            if (x - x_dir, y, z) in obs and (x - x_dir, y, z + z_dir) not in obs: return True
            if (x, y, z - z_dir) in obs and (x + x_dir, y, z - z_dir) not in obs: return True
            # consider Y sides
            if (x, y + 1, z) in obs and (x + x_dir, y + 1, z + z_dir) not in obs: return True
            if (x, y - 1, z) in obs and (x + x_dir, y - 1, z + z_dir) not in obs: return True

        # YZ-diagonal
        if y_dir and z_dir and not x_dir:
            # same-plane checks in YZ
            if (x, y - y_dir, z) in obs and (x, y - y_dir, z + z_dir) not in obs: return True
            if (x, y, z - z_dir) in obs and (x, y + y_dir, z - z_dir) not in obs: return True
            # consider X sides
            if (x + 1, y, z) in obs and (x + 1, y + y_dir, z + z_dir) not in obs: return True
            if (x - 1, y, z) in obs and (x - 1, y + y_dir, z + z_dir) not in obs: return True

        # 3-axis diagonal (XYZ)
        if x_dir and y_dir and z_dir:
            # block behind along any single axis + forward in the other two free
            if (x - x_dir, y, z) in obs and (x - x_dir, y + y_dir, z + z_dir) not in obs: return True
            if (x, y - y_dir, z) in obs and (x + x_dir, y - y_dir, z + z_dir) not in obs: return True
            if (x, y, z - z_dir) in obs and (x + x_dir, y + y_dir, z - z_dir) not in obs: return True

        return False

        #         # horizontal
        #         if x_dir and not y_dir:
        #             if (x, y + 1) in self.obstacles and (x + x_dir, y + 1) not in self.obstacles:
        #                 return True
        #             if (x, y - 1) in self.obstacles and (x + x_dir, y - 1) not in self.obstacles:
        #                 return True
        #
        ...
        #             if (x + 1, y) in self.obstacles and (x + 1, y + y_dir) not in self.obstacles:
        #                 return True
        #             if (x - 1, y) in self.obstacles and (x - 1, y + y_dir) not in self.obstacles:
        #                 return True
        #
        #         # diagonal
        #         if x_dir and y_dir:
        #             if (x - x_dir, y) in self.obstacles and (x - x_dir, y + y_dir) not in self.obstacles:
        #                 return True
        #             if (x, y - y_dir) in self.obstacles and (x + x_dir, y - y_dir) not in self.obstacles:
        #                 return True
        #
        #         return False
